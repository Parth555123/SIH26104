...
Window is 4 s (AASIST's published operating point). No score for the first ~4 s — scorer emits `score: null` / state `OK` ("listening") until the buffer fills.
...
- EER must be computed by interpolation (`scipy.optimize.brentq`), not `min(abs(fpr-fnr))`. **Spoof = positive class (label 1); score = P(spoof)**, higher = more synthetic. `eval/compute_eer.py` warns if a run inverts (EER > 50%).
